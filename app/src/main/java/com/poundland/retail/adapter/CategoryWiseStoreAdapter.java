...
public class CategoryWiseStoreAdapter extends RecyclerView.Adapter<CategoryWiseStoreAdapter.ViewResource> {

    Context mContext;
    DrawerListner drawerListner;
    private List<ProductByCategoryResponseModel.VenuesBean> data;
    private LayoutInflater mInflater;
    private PrefManager prefManager;

    public CategoryWiseStoreAdapter(Context mContext, List<ProductByCategoryResponseModel.VenuesBean> data, DrawerListner drawerListner) {
    ...
    @NonNull
    @Override
    public CategoryWiseStoreAdapter.ViewResource onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_cell_home_top_venue, parent, false); //layout_cell_top_venue

        return new CategoryWiseStoreAdapter.ViewResource(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final CategoryWiseStoreAdapter.ViewResource holder, int position) {
        holder.binding.tvVenueName.setText(data.get(position).getVenue_name());

        if (data.get(position).isIsWishlisted())
        ...
        }else holder.binding.tvOffers.setText(data.get(position).getTotal_offers() + " " + mContext.getString(R.string.offer));

        holder.binding.tvDistance.setText(data.get(position).getDistance() + mContext.getString(R.string.miles));
        ...
        // holder.binding.tvCategoryName.setText(data.get(position).getVenue_name());
    ...
    private void likeDislike(CategoryWiseStoreAdapter.ViewResource holder, int adapterPosition, String venue_id) {
                    ...
                    try {
                        if (response.isSuccessful()) {
                            ...
                            if (venueDetailResponseModel.isStatus()) {
                                if (data.get(adapterPosition).isIsWishlisted()) {
                                    ...
                                    data.get(adapterPosition).setIsWishlisted(false);
                                } else {
                                    holder.binding.ivFavo.setColorFilter(ContextCompat.getColor(mContext, R.color.color_light_red), android.graphics.PorterDuff.Mode.SRC_IN);
                                    data.get(adapterPosition).setIsWishlisted(true);
                                }
