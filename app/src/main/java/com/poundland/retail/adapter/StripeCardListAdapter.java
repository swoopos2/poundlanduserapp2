...
public class StripeCardListAdapter extends RecyclerView.Adapter<StripeCardListAdapter.ViewResource> {

    private Context mContext;
    private List<StripeGetCardDetailsResponseModel.DataBean> data;
    private LayoutInflater mInflater;
    DrawerListner drawerListner;

    public StripeCardListAdapter(Context mContext, List<StripeGetCardDetailsResponseModel.DataBean> data, DrawerListner drawerListner) {
    ...
    @NonNull
    @Override
    public StripeCardListAdapter.ViewResource onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_cell_payment_details, parent, false);

        return new StripeCardListAdapter.ViewResource(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final StripeCardListAdapter.ViewResource holder, int position) {
        ...
        holder.binding.tvCardName.setText("********" + data.get(position).getLast4());
        if (data.get(position).getBrand()!=null)
        if (data.get(position).getBrand().equalsIgnoreCase(mContext.getString(R.string.visa))) {
            holder.binding.ivItemImage.setBackground(ContextCompat.getDrawable(mContext, R.drawable.ic_visa));
        } else if (data.get(position).getBrand().equalsIgnoreCase(mContext.getString(R.string.mastercard))) {
            holder.binding.ivItemImage.setBackground(ContextCompat.getDrawable(mContext, R.drawable.cio_ic_mastercard));
        } else if (data.get(position).getBrand().equalsIgnoreCase(mContext.getString(R.string.maestro_card))) {
